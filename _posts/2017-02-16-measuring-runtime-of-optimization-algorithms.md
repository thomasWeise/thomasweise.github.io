...
In other words, if a TSP instance has $n$~cities, then the worst-case runtime of any exact TSP solver is in~$O(2^n)$.
...
Runtime measurements for single problem instances are then uninteresting, interesting is the comparison over several problem instances of different scales~$n$: How does the runtime grow problem scale~(remember $O(2^n)$?)&hellip;
...
If we have an EA with some standard mutation and crossover operators, then generating a new tour will probably also take something in the range of~$O(n)$ steps.
Maybe~$O(n^2)$ for complicated crossover operators.
Let's stick with~$O(n)$ for now.
In other words, creating and evaluating a new solution may happen together in $O(n)$~steps if we are lucky.
...
This means we can generate and evaluate a new solution under such a "swap" move in~$O(1)$~distance computations.
...
In our previous example, we assumed that an EA could perform a crossover or off-the-shelf mutation operation in~$O(n)$~steps.
...
The ant makes, in total, $O(n^2)$~such decisions.
And to calculate the tour length of a new tour, generated in~$O(1)$~steps by a swap operator, takes at most $8$~distance calculations.

In other words, in these three examples, the number of city-to-city distances we need to compute during the creation of a new solution is in the same big-$O$ class as the fine-grained algorithm steps needed for this process.
...
Reversing a sub-tour of length~$1<m<n$ may be implemented in~$O(m)$ steps while the new tour length is computed in~$O(1)$&hellip;
