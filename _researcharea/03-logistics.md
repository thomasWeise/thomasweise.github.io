...
Logistics is a huge area that poses a wide range of different {% include taglink.liquid tag="optimization" %}  problems and belongs to the wider field of {% include taglink.liquid tag="or" tag_text="Operations Research" %}.
...
The Quadratic Assignment Problem&nbsp;({% include taglink.liquid tag="QAP" %}) is located somewhat on the outer skirts of logistics.
Here, we have a set of locations and a same-sized set of workstations.
We need to assign workstations to locations.
We know the material flow between each pair of workstations&nbsp;(given via a matrix) and the distances between locations&nbsp;(given as a second matrix).
The goal is to find the assignment where the sum over the product of workstation distances and flows is minimized.
As such, the QAP is a generalization of the TSP.
{::nomarkdown}{%- assign papers=site.data.publications | where_exp: "item", "item.tags contains 'QAP'" -%}{%- include publications.liquid papers=papers shorter=true -%}{:/}
